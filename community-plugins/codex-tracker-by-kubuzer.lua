...
blue={r=.4,g=.8,b=1},
green={r=.2,g=1,b=.2},
lightGreen={r=.2,g=1,b=.4},
yellow={r=1,g=.85,b=.25},
orange={r=1,g=.75,b=.3},
}
function on_init()
	if farever and farever.store then
		...
		expMode=farever.store.get("expModeState", false)
	end
end
...
local function make_color_text(color,text)
	local r,g,b= color.r or color[1], color.g or color[1], color.b or color[1]
...
function on_render()
	...
	if farever.target.exists() then
		...
		if current_target_kind ~= "" then
			...
			if codex_info then
				...
				make_color_text({.65}, "Target:")
				imgui.same_line()
				make_color_text(fontColor.yellow, display_name .. lvl_str)

				if codex_info.completed or codex_info.state == "complete" then
					imgui.same_line()
					make_color_text(fontColor.green, " - [100% COMPLETED]")
					remove_waypoint_for_mob(display_name, current_target_kind)
				else
					imgui.same_line()
					if pct >= 0.85 then
						make_color_text(fontColor.lightGreen, string.format(" -  %d / %d", progress, max_val))
					else
						make_color_text({.7}, string.format(" -  %d / %d", progress, max_val))
					end
				...
				if clean_cat ~= "" then
					 make_color_text({.55}, "Category:")
					imgui.same_line()
					make_color_text({.75}, clean_cat)
				end
			else
				make_color_text({.65}, "Target:")
				imgui.same_line()
				make_color_text(fontColor.yellow, current_target_kind)
				make_color_text({.65}, "Codex entry: Not in Bestiary / Unknown")
			end
		end
	else
		make_color_text({.6}, "Select a target to view live Codex progress.")
	end
	...
	make_color_text(fontColor.blue, "--- " .. zone_badge .. header_count_str .. " ---")
	imgui.spacing()

	if #filtered_list == 0 then
		if total_tracked_count > 0 then
			make_color_text(fontColor.green, "All tracked Bestiary entries completed!")
		else
		...
		for i = 1, math.min(#filtered_list, display_limit) do
			...
			make_color_text(fontColor.orange, string.format("%s%s", lvl_prefix, name))
